...
/// 
/// **IMPORTANT**: This test is REVERSIBLE - it initializes all counters to 0 in cleanup.
/// However, it requires hardware access via USB transport.
/// 
/// Run with: cargo test --test lt_test_rev_mcounter -- --nocapture --test-threads=1

use dummy_pin::DummyPin;
use rand_core::{OsRng, RngCore};
use tropic01::{MCounterIndex, Tropic01, X25519Dalek, MCOUNTER_VALUE_MAX};
use tropic01::keys::{SH0PRIV_PROD, SH0PUB_PROD};
...
fn cleanup_counters(tropic: &mut Tropic01<UsbDevice, DummyPin>) -> Result<(), Box<dyn std::error::Error>> {
...
fn test_mcounter_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (idx, index) in all_mcounter_indices().iter().enumerate() {
        ...
        assert_eq!(val, init_val, "Counter {} init failed: expected {}, got {}", idx, init_val, val);
        println!("[Counter {}] Verified initialization: {}", idx, val);

        // Re-initialize with same value (should succeed but not change value)
        println!("[Counter {}] Re-initializing with same value (should be idempotent)...", idx);
        tropic.mcounter_init(*index, init_val)?;
        let val_after_reinit = tropic.mcounter_get(*index)?;
        assert_eq!(val_after_reinit, init_val, "Counter {} changed after re-init", idx);
        ...
        while expected_val > 0 && decrement_count < MAX_DECREMENTS {
            ...
            assert_eq!(current, expected_val, 
                "Counter {} mismatch: expected {}, got {}", idx, expected_val, current);
        ...
        println!("[Counter {}] After {} decrements: {}", idx, decrement_count, final_val);
    }
    ...
    for (idx, index) in all_mcounter_indices().iter().enumerate() {
        ...
        println!("\n[Counter {}] Initializing with small value: {}", idx, small_init_val);
        ...
        // Verify we're at 0
        let final_val = tropic.mcounter_get(*index)?;
        assert_eq!(final_val, 0, "Counter {} should be 0, got {}", idx, final_val);
        println!("[Counter {}] Successfully decremented to 0", idx);

        // Try to decrement when at 0 (should fail)
        println!("[Counter {}] Attempting to decrement below 0 (should fail)...", idx);
