...
fn print_fw_header(fw_bank_info: &[u8]) -> Result<(), anyhow::Error> {
    if fw_bank_info.len() == L2_GET_INFO_FW_HEADER_SIZE_BOOT_V1 {
        println!(
            "    Bootloader v1.0.1 detected, reading {:?} header\r\n",
            L2_GET_INFO_FW_HEADER_SIZE_BOOT_V1
        );
    } else if fw_bank_info.len() == L2_GET_INFO_FW_HEADER_SIZE_BOOT_V2 {
        println!(
            "    Bootloader v2.0.1 detected, reading {:?} header\r\n",
            L2_GET_INFO_FW_HEADER_SIZE_BOOT_V2
        );
        let header_boot_info = LtHeaderBootV2::try_from(&fw_bank_info[..]).map_err(|e| {
            println!("Failed to parse header boot: {}", e);
            anyhow::anyhow!("Header boot parsing error: {}", e)
        })?;
        println!("{header_boot_info}");
    } else if fw_bank_info.len() == L2_GET_INFO_FW_HEADER_SIZE_BOOT_V2_EMPTY_BANK {
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let riscv_fw_ver = tropic.get_info_riscv_fw_ver().map_err(|e| {
    ...
    let spect_fw_ver = tropic.get_info_spect_fw_ver().map_err(|e| {
    ...
    //LT_LOG_ERROR("Failed to reboot into MAINTENANCE mode, ret=%s",
    // lt_ret_verbose(ret));

    println!(
        "Reading RISC-V FW version (during maintenance chip actually returns bootloader version):"
    );
    let riscv_fw_ver = tropic.get_info_riscv_fw_ver().map_err(|e| {
        eprintln!("Failed to get RISC-V FW version: {:?}", e);
        anyhow::anyhow!("Failed to get RISC-V FW version: {:?}", e)
    ...
    let _ = print_fw_header(&fw_bank_info);
